...
let output = "import { Category, MenuItem } from './types';\\n\\n";
output += "export const COLORS = {\\n";
output += "  primary: '#F57C00',\\n";
output += "  secondary: '#262626',\\n";
output += "  success: '#22c55e',\\n";
output += "  danger: '#ef4444',\\n";
output += "};\\n\\n";

output += "export const INITIAL_CATEGORIES: Category[] = [\\n";
data.menu.forEach((cat, i) => {
  output += "  { id: 'cat" + (i+1) + "', name: '" + cat.category + "' },\\n";
});
output += "];\\n\\n";

output += "export const INITIAL_MENU_ITEMS: MenuItem[] = [\\n";
...
data.menu.forEach((cat, catIdx) => {
  ...
  cat.items.forEach(item => {
    ...
    } else if (item.prices && item.prices.half && item.prices.full) {
    ...
    } else if (item.prices && item.prices.full) {
